...
var umdify = function(contents, moduleName) {
	...
	if (requireStatements.requires.length > 0 ){
		output += '\t\tmodule.exports = factory(\t' + requireStatements.requires.join(',\n\t\t\t') + ');';
	} else {
	...
	// if (moduleName) {
		...
		if (requireStatements.varNames.length > 0) {
			output += '\t\troot.returnExports = factory(root.' +requireStatements.varNames.join(',\n\t\t\troot.')+ ');\n';
		} else {
	...
	// }
	if (requireStatements.varNames.length > 0) {
		output += '}(this, function(' + requireStatements.varNames.join(',\n\t\t') + '\n\t) {';
	} else {
